...
include(scriptsdir("guitar_model.jl"))
...
function render_graph(metagraph; render_vertices=true, svg=true)
    if svg
        @svg begin
...
function create_diffeq_graph(n, initial_array, initial_gradients, potential_function, α, β, γ; expansion_rate=0.05, interior_func=x->true)
    ...
    for child_vertex in length(initial_array)+1:n
        ...
        while true
            ...
            if interior_func((child_position .- topleft_corner) ./ difference_width)
            ...
            tries += 1
            if tries > 100
                throw(ErrorException("too many tries to find a connection :("))
            end
        end
...
mg = top_bottom_proto_rrt_potential(10000;expansion_rate=1.0)
render_graph(mg; render_vertices=true, svg=false)

function guitar_proto_rrt_potential(n, α, β, γ; expansion_rate=1.5)
    charge_constant = 100.0
    topleft_corner = GB.Point2(-300.0, -300.0)
    bottomright_corner = GB.Point2(300.0, 300.0)
    left_point = GB.Point2(0.5, 0.2) .* (bottomright_corner .- topleft_corner) .+ topleft_corner
    right_point = GB.Point2(0.25, 0.66) .* (bottomright_corner .- topleft_corner) .+ topleft_corner
    potential_function(p) = charge_constant .* (-1.0 ./ (norm(p .- left_point) + 1e-1) .+ 1.0 ./ (norm(p .- right_point) + 1e-1))
    initial_array = [left_point, right_point]
    initial_gradients = [GeometryBasics.Point2(0.0, 1.0), GeometryBasics.Point2(0.0, 1.0)]
    interior_func = inside_guitar_function
    create_diffeq_graph(n, initial_array, initial_gradients, potential_function, α, β, γ; expansion_rate=expansion_rate, interior_func)
end

mg = guitar_proto_rrt_potential(100000, 0.3, 1.0, 1.2;expansion_rate=1.0)
render_graph(mg; render_vertices=true, svg=true)